...
(^#) ::  Int -> Int -> Int
_ ^# 0 = 1 -- raise to power of 0 is always 1
b ^# e = b * (b ^# (e - 1))

{-
2^3
= {applying definition of ^}  2 * (2^2)
= {applying definition of ^}  2 * 2 * (2^1)
= {applying definition of ^}  2 * 2 * 2 * (2^0)
= {applying definition of ^}  2 * 2 * 2 * 1
= {applying last *}           2 * 2 * 2
= {applying last *}           2 * 4
= {applying last *}           8
...
{-
length [1,2,3]
= {applying length}  1 + length [2, 3]
= {applying length}  1 + 1 + length [3]
= {applying length}  1 + 1 + 1 + length []
= {applying length}  1 + 1 + 1 + 0
= {applying +}       1 + 1 + 1
= {applying +}       1 + 2
= {applying +}       3
...
...
drop 3 [1,2,3,4,5]
= {applying drop}    drop 2 [2,3,4,5]
= {applying drop}    drop 1 [3,4,5]
= {applying drop}    drop 0 [4,5]
= {applying drop}    [4,5]
...

init [1,2,3]
= {applying init}    1 : init [2,3]
= {applying init}    1 : 2 : init [3]
= {applying init}    1 : 2 : []
= {applying :}       [1,2]
-}
...
merge :: Ord a => [a] -> [a] -> [a]
merge [] ys = ys                         -- If first list is empty return second
merge xs [] = xs                         -- If second list is empty return first
merge (x:xs) (y:ys)
    | x <= y      = x:merge xs (y:ys)
...
msort [x] = [x]                              -- single element is also sorted by definition
